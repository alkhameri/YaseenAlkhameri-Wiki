...
export async function middleware(req: NextRequest, event: NextFetchEvent) {
  ...
  const analyticsEnabled = hasSupabaseConfig() && hasVisitorCookieConfig();
  const existingVidToken = analyticsEnabled
    ? req.cookies.get(VID_COOKIE)?.value
    : undefined;
  let vid = analyticsEnabled ? await verifyVid(existingVidToken) : null;
  let mintedVidToken: string | null = null;
  if (analyticsEnabled && !vid) {
    vid = newVid();
  ...
  if (!analyticsEnabled || !vid || !isTrackablePath(pathname) || isPrefetch(req)) {
    return res;
  }
